...
function SyncPage() {
  ...
  async function devHehez() {
    ...
    const endpoint =
      url + '/project/commit' + '?pid=' + pid + '&cno=' + syncCommit
      ...
      {import.meta.env.DEV ? (
        <div className="flex flex-col gap-y-2">
          ...
          <div className="flex flex-row gap-x-2">
            <Input
              onChange={(e) => {
                setSyncCommit(e.target.value)
              }}
              value={syncCommit}
            />
            <Button onClick={devSync} disabled={syncInProgress}>
              Sync at commit
            </Button>
            <Button onClick={devHehez} variant={'secondary'}>
          ...
          <div className="flex flex-row gap-x-2">
            ...
            <Button
              onClick={async () => {
                await invoke('delete_project', { pid: parseInt(pid) })
              }}
              variant={'destructive'}
            >
              Delete Project
