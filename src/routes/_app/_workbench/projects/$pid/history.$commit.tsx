...
import { HistorySearch } from 'history.index'
...
function Commit() {
  ...
  if (data.response != 'success') {
    console.log(data)
    return <div>An error occurred while fetching data, create an issue</div>
  }
    ...
    <div className="flex flex-col items-start space-y-2">
      <div className="flex flex-row items-center place-items-start space-x-4">
        ...
        <div className="font-semibold text-xl">
          Project Update {data.body.description.commit_number} at{' '}
          {d.toLocaleString()}
        </div>
        {/*
            <Dialog>
      ...
      <div className="flex flex-row grow border p-2 rounded-md w-full text-sm">
        {data.body.description.comment.length == 0
          ? 'No comment provided'
          : data.body.description.comment}
      </div>
      <div className="text-md">
        {data.body.description.num_files} file
        {data.body.description.num_files > 1 ? 's' : ''} updated by{' '}
        {data.body.description.author}:
      </div>
